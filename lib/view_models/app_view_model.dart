import 'package:flutter/material.dart';

import '../models/task_model.dart';
import '../models/user_model.dart';
import '../repository/repository.dart';

class AppViewModel extends ChangeNotifier {
  List<Task> tasks = <Task>[];
  AppViewModel(List<Task> list){
    tasks = list;
  }

  Repository newRep = Repository();



  // Color clrLvl1 = Colors.grey.shade50;
  // Color clrLvl2 = Colors.grey.shade200;
  // Color clrLvl3 = Colors.grey.shade800;
  // Color clrLvl4 = Colors.grey.shade900;

  Color clrLvl1 = Colors.grey.shade50;
  Color clrLvl2 = Colors.grey.shade200;
  Color clrLvl3 = Colors.grey.shade800;
  Color clrLvl4 = Colors.grey.shade900;


  int get numTasks => tasks.length;

  int get numTasksRemaining => tasks.where((task) => !task.complete).length;

  String get username => user.username;

  //User data section
  User user = User("Md. Rakibul Islam");
  void updateUsername(String newUserName) {
    user.username = newUserName;
    newRep.setUserName(newUserName);
    notifyListeners();
  }

  Future<String> get userName => newRep.getUsername();

  // Task data section

  //add data

  void addTask(Task newTask) {
    print(tasks);
    tasks.add(newTask);
    notifyListeners();
    newRep.setTaskList(tasks);
  }

  //set data

  void setTaskValue(int taskIndex, bool taskValue) {
    tasks[taskIndex].complete = taskValue;
    notifyListeners();
    newRep.setTaskList(tasks);
  }

  //get data

  bool getTaskValue(int taskIndex) {
    return tasks[taskIndex].complete;
  }

  String getTaskTitle(int taskIndex) {
    return tasks[taskIndex].title;
  }

  String getTime(int taskIndex){
    return tasks[taskIndex].time;
  }

  //delete data

  void deleteTask(int taskIndex) {
    tasks.removeAt(taskIndex);
    notifyListeners();
    newRep.setTaskList(tasks);
  }

  void deleteAllTasks() {
    tasks.clear();
    notifyListeners();
    newRep.deleteAll();
  }

  void deleteCompletedTasks() {
    tasks = tasks.where((task) => !task.complete).toList();
    notifyListeners();
    newRep.setTaskList(tasks);
  }

  //task details
  bool check = false;
  void onTapText(bool tap){
    check = tap;
    notifyListeners();
  }

  void setUpdateText(String text, int index){
    tasks[index].title = text;
    notifyListeners();
    newRep.setTaskList(tasks);
  }


// bottom sheet
  void bottomSheetBuilder(Widget bottomSheetView, BuildContext context) {
    showModalBottomSheet(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(40)),
        clipBehavior: Clip.antiAliasWithSaveLayer,
        context: context,
        builder: ((context) {
          return bottomSheetView;
        }));
  }
}